...
# }
#-----------------------------------------------
#save(all_pairs, file="all_pairs.RData")  вроде не надо
...
repeat{
  ...
  #-------------------------------------------------------------------------    
    ...
    if (x > 0.001){
      for (j in  1:3){
        Symbol <- Couples (bundle[[1]][j], bundle[[1]][j+1], all_pairs)
        if (Symbol [[2]] == FALSE){
          amount <- sell(Symbol[[1]], amount)
          cat("sell", Symbol[[1]], amount)
        }
        else if (Symbol[[2]] == TRUE){
          amount <- buy(Symbol[[1]], amount) 
          cat("buy", Symbol[[1]], amount)
        }
      }
      break
    }
  ...
  for (i in 1:length(list_profits)){
